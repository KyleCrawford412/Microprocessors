//
//  ADC Code for DTMF Project
//  Configures the ADC for Repeated Single Channel Mode
//  Uses Timer B0 to set the sample frequency to 5.2KHz
//
...
uint16_t adcIndex = 0;  // index for ADC buffer
uint8_t  adcFlag = 0;   // Flag to indicate that adc buffer is full

void adc_init(void)
{


    //Set up Timer B.  Used as sample clock
    // Sampling Rate is set to 5.2 kHz.  It will not go much higher than this with SMCLK = 8 Mhz

    Timer_B_initCompareModeParam initCompParam = {0};
    initCompParam.compareRegister = TIMER_B_CAPTURECOMPARE_REGISTER_1;
    ...
    initCompParam.compareValue = 0x00C0;
    ...
    initUpParam.clockSourceDivider = TIMER_B_CLOCKSOURCE_DIVIDER_6;
    initUpParam.timerPeriod = 0x00FF;
    ...
    * Use Timer B0 for Sample Clock
    * USE MODOSC 5MHZ Digital Oscillator as clock source
    * Use default clock divider/pre-divider of 1
    * Not use internal channel
    */
   	ADC12_B_initParam initParam = {0};
    initParam.sampleHoldSignalSourceSelect = ADC12_B_SAMPLEHOLDSOURCE_3;
    ...
    /*
    * Base address of ADC12B Module
    * For memory buffers 0-7 sample/hold for 4 lock cycles
    ...
    */
    ...
    configureMemoryParam.inputSourceSelect = ADC12_B_INPUT_A0;     // Need to set this to the correct analog channel
    ...
    //Enable ADC memory buffer 0 interrupt
    ADC12_B_enableInterrupt(ADC12_B_BASE,ADC12_B_IE0,0,0);

    // Clear ADC variables using by main
    adcFlag = 0;
    adcIndex = 0;
...
// This is the ISR for the ADC
// It assumes that an interrupt is set by a write into ADC12BMEM0
...
__interrupt void ADC12_ISR(void)
{
  switch(__even_in_range(ADC12IV,12))
  {
    ...
    case 12:                                // Vector 12:  ADC12BMEM0 Interrupt
        ...
        // Check if adcFull flag is set to empty
      if(adcFlag == 0){
        // if yes, get next ADC result
          adcBuffer[adcIndex] =  ADC12_B_getResults(ADC12_B_BASE, ADC12_B_MEMORY_0);
          if(adcIndex < 256){
              adcIndex++;
          }else{
//   adc buffer is full.  Set flag and reset adc index to 0
              adcFlag = 1;
              adcIndex = 0;
